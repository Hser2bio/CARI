...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto cariAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(cariAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(cariAdd);
