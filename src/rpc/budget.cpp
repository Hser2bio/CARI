...
UniValue preparebudget(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->CreateBudgetFeeTX(wtx, nHash, keyChange, false)) { // 50 CARI collateral for proposal
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"TotalPayment\": xxx.xxx,      (numeric) Total payment amount in CARI\n"
            "    \"MonthlyPayment\": xxx.xxx,    (numeric) Monthly payment amount in CARI\n"
            ...
            "    \"Allotted\": xxx.xxx,           (numeric) Amount of CARI allotted in current period\n"
            "    \"TotalBudgetAllotted\": xxx.xxx (numeric) Total CARI allotted\n"
            "  }\n"
            "  ,...\n"
            "]\n"
